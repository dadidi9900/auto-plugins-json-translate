<template>
  <div class="translation-container">
    <div class="header">
      ...
      <p class="subtitle">使用 AI 技术快速翻译您的 JSON 语言文件。快速、准确、易用。</p>
    </div>
      ...
      <div class="config-section">
        ...
        >
          <div class="upload-content">
            <el-icon class="el-icon--upload"><upload-filled v-if="!currentFile" /><document v-else /></el-icon>
        ...
        <el-form :model="form" class="config-form">
          <el-form-item class="api-key-item">
            <el-input v-model="form.apiKey" type="password" placeholder="请输入 DeepSeek API Key">
              <template #prefix>
                <el-icon><key /></el-icon>
              </template>
            </el-input>
          ...
          <el-form-item class="source-language-item">
            <el-select v-model="form.sourceLanguage" placeholder="请选择源语言" class="language-select">
              <el-option
          ...
          <el-form-item class="action-buttons">
            <el-button type="primary" @click="startTranslation" :disabled="!canStart" class="start-button">
              开始翻译
            </el-button>
          </el-form-item>
        </el-form>
      ...
      <div class="preview-section">
        <div class="preview-container">
          <div class="preview-header">
            <h3>原始 JSON</h3>
            <p class="preview-desc">提示：支持实时预览和格式化 JSON 文件，您可以直接复制内容</p>
          </div>
          <pre class="language-json"><code v-html="formatJson(jsonContent)"></code></pre>
        </div>
        ...
        <div class="preview-container">
          ...
          <pre class="language-json streaming-preview"><code ref="streamingCode" v-html="formatStreamingContent(streamingContent[form.languages[0]] || {})"></code></pre>
        </div>
        ...
        <div class="preview-container preview-container-large">
          ...
          <pre class="language-json"><code v-html="formatJson(getTranslationResult(activeTab))"></code></pre>
          <div class="custom-tabs">
            ...
            >
              {{ lang.label }}
              <el-tag v-if="existingLanguages.has(lang.value)" size="small" type="warning" class="translated-tag">已翻译</el-tag>
            </div>
          </div>
        </div>
      </div>
  ...
  <!-- 自定义合并结果弹窗 -->
  <div v-if="showMergeDialog" class="custom-dialog-overlay" @click="showMergeDialog = false">
    <div class="custom-dialog" @click.stop>
      ...
      <div class="dialog-content">
        <pre class="language-json"><code v-html="formatJson(mergedResult || {})"></code></pre>
      </div>
      <div class="dialog-footer">
        <el-button @click="showMergeDialog = false">关闭</el-button>
        <el-button type="primary" @click="copyTranslation">复制结果</el-button>
        <el-button type="success" @click="exportTranslation">下载结果</el-button>
      </div>
    </div>
  </div>
</template>
...
import { languages, getLanguageLabel as getLanguageLabelFromConfig } from '../config/languages'
...
const loadFromCache = (language: string): boolean => {
  ...
  const cachedData = localStorage.getItem(`translation_cache_${fileHash.value}_${language}`)
  if (cachedData) {
    try {
...
const saveToCache = (language: string, content: Record<string, any>) => {
  ...
  try {
    localStorage.setItem(`translation_cache_${fileHash.value}_${language}`, JSON.stringify(content))
    // 更新已翻译语言集合
    existingLanguages.value.add(language)
...
// 监听源语言变化，自动从目标语言中移除
watch(() => form.value.sourceLanguage, (newSourceLang) => {
  if (newSourceLang) {
    form.value.languages = form.value.languages.filter(lang => lang !== newSourceLang)
  }
})
  ...
  reader.onload = (e) => {
    try {
      ...
      if (loadedLanguages.length > 0) {
        const languageLabels = loadedLanguages.map(lang => getLanguageLabel(lang)).join('、')
...
const startTranslation = async () => {
  ...
  streamingContent.value = {}
  mergedResult.value = null // 清除之前的合并结果
  ...
  while (currentBlockIndex.value < blocks.value.length) {
    ...
    for (const language of form.value.languages) {
      ...
      const result = await service.translateBlock(block, language, (content) => {
        try {
      ...
      if (result.status === 'success') {
        ...
        translatedContent.value[language] = {
          ...translatedContent.value[language],
          ...Object.fromEntries(
            Object.entries(result.translations).map(([key, value]) => [
              key,
        ...
        // 保存到本地缓存
        saveToCache(language, translatedContent.value[language])
      } else {
...
const mergeTranslations = () => {
  ...
  allKeys.forEach(key => {
    ...
    if (jsonContent.value[key][form.value.sourceLanguage]) {
      result[key][form.value.sourceLanguage] = jsonContent.value[key][form.value.sourceLanguage]
    }
...
const exportTranslation = () => {
  ...
  try {
    ...
    link.href = url
    link.download = `translation_${new Date().toISOString().split('T')[0]}.json`
...
watch(() => streamingContent.value[form.value.languages[0]], () => {
  nextTick(() => {
...
<style scoped>
/* 移除所有重复的样式，因为它们已经在 translation-form.css 中定义 */
</style>