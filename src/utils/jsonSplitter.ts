...
export class JsonSplitter {
  ...
  private static readonly MAX_OUTPUT_TOKENS = 8000;
  ...
  static splitJson(jsonContent: Record<string, any>, maxTokens: number = this.MAX_OUTPUT_TOKENS - this.RESERVED_TOKENS): TranslationBlock[] {
    const blocks: TranslationBlock[] = [];
    let currentBlock: Record<string, any> = {};
    let currentTokens = 0;

    // 计算系统提示词的基础 token 数
    const basePrompt = "你是一个专业的翻译助手。请将以下 JSON 对象中的值翻译成目标语言。保持 JSON 结构不变，只翻译值部分。请直接返回翻译后的 JSON 对象，不要包含任何解释。";
